...
function PageOption({number, isActive,isSelected, onClick}) {
    return <div className={`${isActive ? "hover:bg-gray-300 hover:font-bold text-gray-800 font-semibold cursor-pointer" :'cursor-not-allowed' } ${isSelected ? 'bg-gray-800 text-white hover:bg-gray-800 font-bold  hover:font-bold' : ''} ${!isActive && !isSelected ? 'opacity-50' : '' }  rounded-full size-[2rem] flex items-center justify-center` } onClick={isActive ? onClick : undefined}>
        {number}
    </div>
}