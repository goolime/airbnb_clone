import { useEffect, useState } from "react"
import { DayPicker } from "react-day-picker"

export function MobileDatePicker({ onFilterChange }) {
    const [selected, setSelected] = useState(undefined);
    const [numberOfMonths, setNumberOfMonths] = useState(3);
    const now = new Date(Date.now());
    const endMonth = new Date(now.getFullYear() + 2, now.getMonth() - 1);
    const startMonth = new Date(now.getFullYear(), now.getMonth());

    function handleSelectedChange(selectedRange) {
        onFilterChange(selectedRange);
        setSelected(selectedRange);
    }


    //console.log(selected)

    return <>
        <div className="flex flex-col  w-full h-[60vh] justify-center">
            <DayPicker
                mode="range"
                selected={selected}
                onSelect={handleSelectedChange}
                numberOfMonths={numberOfMonths}
                hideNavigation={true}
                navLayout="around"
                fixedWeeks={true}
                disabled={{ before: now, after: new Date(now.getFullYear() + 2, now.getMonth(), now.getDate()) }}
                startMonth={startMonth}
                endMonth={endMonth}
                className="overflow-scroll scrollbar-hide w-full h-[60vh]"
                classNames={{
                    months: "flex flex-col gap-2",
                    month_caption: "mb-2",
                    caption_label: "font-semibold text-[#222222] text-md",
                    day_button: "w-full h-full rounded-full hover:border hover:border-1 hover:border-[#dddddd]",
                    weekday: "text-[#6a6a6a] font-normal",
                    button_previous: "m-2 p-2 rounded-full hover:bg-gray-200 absolute left-0 top-1/2 -translate-y-1/2 aria-disabled:hidden ",
                    button_next: "m-2 p-2 rounded-full hover:bg-gray-200 absolute right-0 top-1/2 -translate-y-1/2 aria-disabled:hidden ",
                    day: "size-[10dvw] text-sm text-[#222222] text-center p-0.5",
                    range_start: "rounded-l-full text-white rangeStart",
                    range_end: "rounded-r-full text-white rangeEnd",
                    selected: "has-[button]:bg-gray-100 selected",
                    disabled: "opacity-20",
                }}
                components={
                    {
                        Day: (props) => {
                            const { day, modifiers, ...tdProps } = props;
                            const Month = day.date.getMonth()
                            if (day.date.getDate() === 1) {
                                tdProps.className += " start-of-month"
                            }
                            if (Month !== (new Date(day.date.getTime() + 1000 * 60 * 60 * 24)).getMonth()) {
                                tdProps.className += " end-of-month"
                            }
                            return <td {...tdProps} />
                        }
                    }
                }
                footer={
                    <button className={`${numberOfMonths >= 24 ? "hidden" : ""} w-full py-3 px-6 text-[#222222] border-1 border-[#222222] rounded-lg text-md font-semibold `}
                        onClick={() => setNumberOfMonths((prev) => Math.min(prev + 3))}>Load more dates</button>
                }
            />

        </div>
    </>
}