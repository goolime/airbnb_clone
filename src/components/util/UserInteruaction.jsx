...
export function UserInteruction({ className, mobileHeight = '75%', isOpen = true, onClose = () => {}, children }) {
    ...
    return <>
        <div className={`fixed inset-0 ${isOpen ? '' : 'hidden'} ${animation ? 'opacity-0' : ''} duration-300 z-999`}>
            ...
            <div className={`absolute bottom-0 bg-white p-6 max-sm:rounded-t-3xl sm:rounded-3xl shadow-lg max-sm:w-full sm:max-w-2xl sm:mx-auto opacity-100 z-1001 sm:relative sm:top-[8%] h-[${mobileHeight}] sm:h-[80%] overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] ${animation ? 'translate-y-[20px] opacity-0' : ''} duration-300 ${className}`}>
